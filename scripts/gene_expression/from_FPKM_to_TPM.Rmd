...
params:
  ...
  output_fpkm: # output fpkm table path
  output_tpm: # output tpm table path
---
...
```{r data, include = FALSE}
	# FPKM
		...
		cancer_types=unique(manifest$project_id)
		fpkm_tmp=lapply(cancer_types, function(x){
				...
				if(length(res_list)>0){
					...
					write.table(res_tab, file=paste(params$output_fpkm, x, "_fpkm.txt", sep=''), quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)
					return(NA)
				} else {
					return(NA)
				}
		...
		fpkm=lapply(cancer_types, function(x){
				res_tmp=fread(paste(params$output_fpkm, "/", x, "_fpkm.txt", sep=''), sep="\t")
				res=as.matrix(res_tmp, rownames=1)
				return(res)
			}
		)
...
```{r write_out, include = FALSE}
	# Write the table
		for(i in 1:length(TPM)){
			write.table(TPM[[i]], file=paste(params$output_tpm, names(TPM)[i], "_tpm.txt", sep=''), quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
		}	
```
