...
```{r bubble_plot, echo = FALSE}
	...
	if(params$bubble_plot!=""){
		...
		if(!is.na(heat_labels)){
			bubble_lab=sapply(cont_ass_tab[,"feat"], function(x){
					p=which(c(cat_features, cont_features)==x)
					return(heat_labels[p])
				}
			)
			cont_ass_tab[,"feat"]=bubble_lab
			cont_ass_tab[,"feat"]=factor(cont_ass_tab[,"feat"], levels=rev(heat_labels))
		}
