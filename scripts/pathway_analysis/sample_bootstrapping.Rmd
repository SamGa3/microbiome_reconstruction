...
params:
  ...
  cont_properties: "" # vector with the continouos properties to be used to select samples
  binning_approach: "" # approach chosen to bin the cointinuous properties
	...
	# Upload of
		...
		# Properties
			if(all(params$cont_properties=="")){
				...
				new_feat=sapply(1:length(params$cont_properties), function(x){
						if(params$binning_approach[[x]][["approach"]]=="property_binning_decision"){
							thr=property_binning_decision(metadata=full_metadata,
																							columns_to_be_tested=params$cont_properties[[x]], 
																							perc_thr=params$binning_approach[[x]][["perc_thr"]], 
																							perc_NA=params$binning_approach[[x]][["perc_NA"]])
							if(length(thr[[1]])==1){
								new_val=sapply(full_metadata[,params$cont_properties[[x]]], function(y){
										if(is.na(y)){
											return("unknown")
										} else if(y>=thr[[1]]){
											return("high")
								...
								size_mode=0.1
								while(size_mode>0.05 & length(thr[[x]])>1){
									modes_values=Modes(full_metadata[,params$cont_properties[[x]]], min.size=size_mode)$modes
									if(length(modes_values)==1){
										size_mode=size_mode-0.005
									} else {
										den=density(full_metadata[,params$cont_properties[[x]]], na.rm=TRUE)
				...
				new_cont_names=sapply(params$binning_approach, function(x){
						x[["new_name"]]
					}
				)
				colnames(new_feat)=new_cont_names
				full_metadata=data.frame(full_metadata, new_feat)
				properties=c(params$properties, new_cont_names)
			}
